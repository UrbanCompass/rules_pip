...
def _get_bin(repository_ctx):
    """Gets the bin path."""
    bin_path = repository_ctx.attr.interpreter
    if "/" not in bin_path:
        bin_path = repository_ctx.which(bin_path)
    if bin_path != None:
        return str(bin_path)
    return _fail("Cannot find %s in PATH" % bin_path)
...
def _python_autoconf_impl(repository_ctx):
    ...
    """
    python_bin = _get_bin(repository_ctx)
    python_include = _get_python_include(repository_ctx)
    python_lib = _get_python_lib_path(repository_ctx)
    ...
    repository_ctx.symlink(python_lib, "lib/")
